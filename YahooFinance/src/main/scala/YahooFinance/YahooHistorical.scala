...
object YahooHistoricalTest extends App{
  var yahooHistorical = new YahooHistorical("G3B.SI",Array(2018,10,29))
  println(yahooHistorical.getDividends())
  println(yahooHistorical.getHistorical())
  println(yahooHistorical.getSplits())
}

class YahooHistorical(ticker: String, start: Array[Int], end: Array[Int]=Array(), interval: String ="1d"){
  val api_url = "https://query1.finance.yahoo.com/v7/finance/download/%s?period1=%s&period2=%s&interval=%s&events=%s&crumb=%s"
  ticker.toUpperCase
  val (crumb, cookies)=getCrumb(ticker)
  ...
  def getData(events: String): String ={
    ...
    val url = api_url.format(ticker,start_epoch,end_epoch,interval,events,crumb)
    println(url)
